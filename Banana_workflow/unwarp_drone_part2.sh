...
GCP_ref_txt=GCP_ref.xyz
GCP_ref_xml=GCP_ref.xml
...
mm3d Campari .*JPG Ground_GCP_Init Ground_RTL_iter2 GCP=[$GCP_ref_xml,0.3,GCPPositionImage.xml,3] AllFree=1 SH=_mini

mm3d ChgSysCo  .*JPG Ground_RTL_iter2 RTLFromExif.xml@SysUTM.xml Ground_GCP_iter2

# 5. Compute the new DEM in the reference coordinate system
mm3d Malt Ortho .*JPG Ground_GCP_iter2 ResolTerrain=0.1 ZoomF=2 EZA=1 DirMEC=MEC-GCP ResolOrtho=0.5

# 5.1 Compute the new Ortho photo
mm3d Tawny Ortho-MEC-GCP RadiomEgal=0

# 6. Finish by running the gdal_translate command
gdal_translate  -a_srs "+proj=utm +zone=32N +ellps=WGS84 +datum=WGS84 +units=m +no_defs" MEC-GCP/Z_Num9_DeZoom2_STD-MALT.tif OUTPUT/DEM_iter2.tif
cp MEC-GCP/Z_Num9_DeZoom2_STD-MALT.tfw MEC-GCP/Correl_STD-MALT_Num_8.tfw
gdal_translate  -a_srs "+proj=utm +zone=32N +ellps=WGS84 +datum=WGS84 +units=m +no_defs" MEC-GCP/Correl_STD-MALT_Num_8.tif OUTPUT/correl_iter2.tif
gdal_translate -a_srs "+proj=utm +zone=32N +ellps=WGS84 +datum=WGS84 +units=m +no_defs" Ortho-MEC-GCP/Orthophotomosaic.tif OUTPUT/Orthophotomosaic.tif
