...
#This file is a workflow for drone images taken at nadir (or close to nadir) with an accompanying CSV file with GNSS location data of the camera position. It was created and tested using images from a Quantum Systems F90+ and should run for all similar drones, and also for more "serious" aerial photography systems.
...
# example:
# ./DroneNadir_ExternalLocationData.sh -e JPG -u "32 +north" -f GNSS_FILE.csv 
...
while getopts "e:x:y:u:v:g:f:sd:pcao:r:z:t:n:h" opt; do
  case $opt in
    h)
      ...
      echo "	-g GNSS_Q        : Quality of embedded GNSS (in m, Def=0.1m, right for RTK GNSS data)"
      echo "	-f GNSS_FILE     : txt file with camera location information"
...
nbIm=0
re='^[+-]?[0-9]+([.][0-9]+)?$'
[ ! -f $GNSS_FILE ] && { echo "$INPUT file not found"; exit 99; }
while read image lon lat alt omega phi kappa accuracyH accuracyV
do	
	if [[ $lon =~ $re ]] ; then
		LON_MID=$(awk "BEGIN {print $LON_MID + $lon}")
		LAT_MID=$(awk "BEGIN {print $LAT_MID + $lat}")
		nbIm=$(awk "BEGIN {print $nbIm + 1}")
	fi
done < $GNSS_FILE

LON_MID=$(awk "BEGIN {print $LON_MID / $nbIm }")
LAT_MID=$(awk "BEGIN {print $LAT_MID / $nbIm }")
echo "Middle of" $nbIm "camera locations : " $LON_MID  $LAT_MID

rm RTL.xml
echo "<?xml version="1.0" ?>                            " >> RTL.xml
echo "<SystemeCoord>                                    " >> RTL.xml
echo "     <BSC>                                        " >> RTL.xml
echo "          <TypeCoord>eTC_RTL</TypeCoord>          " >> RTL.xml
echo "          <AuxR>"$LON_MID"</AuxR>                 " >> RTL.xml
echo "          <AuxR>"$LAT_MID"</AuxR>                 " >> RTL.xml
echo "          <AuxR>0</AuxR>                          " >> RTL.xml
echo "          <ByFile>false</ByFile>                  " >> RTL.xml
echo "     </BSC>                                       " >> RTL.xml
echo "     <BSC>                                        " >> RTL.xml
echo "          <TypeCoord>eTC_WGS84</TypeCoord>        " >> RTL.xml
echo "          <ByFile>false</ByFile>                  " >> RTL.xml
echo "          <AuxRUnite>eUniteAngleDegre</AuxRUnite> " >> RTL.xml
echo "     </BSC>                                       " >> RTL.xml
echo "</SystemeCoord>                                   " >> RTL.xml                                                       
...
echo "mm3d OriConvert \"#F=N X Y Z W P K Ix Iz\""$GNSS_FILE"RAWGNSS_N ChSys=DegreeWGS84@RTL.xml MTD1=1 NameCple=FileImagesNeighbour.xml DN=$DN OkNoIm=1"
mm3d OriConvert "#F=N X Y Z W P K Ix Iz" $GNSS_FILE RAWGNSS_N ChSys=DegreeWGS84@RTL.xml MTD1=1 NameCple=FileImagesNeighbour.xml DN=$DN OkNoIm=1
